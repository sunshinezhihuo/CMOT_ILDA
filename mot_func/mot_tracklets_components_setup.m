...
%     lx = detections(tmp_idx).x(ass_idx(tmp_idx)); 
%     ly = detections(tmp_idx).y(ass_idx(tmp_idx));
%     w = detections(tmp_idx).w(ass_idx(tmp_idx)); 
%     h = detections(tmp_idx).h(ass_idx(tmp_idx));
%     [cx,cy] = LeftToCenter(lx,ly,h,w);
%     Trk(noft).state{tmp_idx}(1,1) = cx; 
%     Trk(noft).state{tmp_idx}(2,1) = cy;
%     Trk(noft).state{tmp_idx}(3,1) = w;
%     Trk(noft).state{tmp_idx}(4,1) = h;
